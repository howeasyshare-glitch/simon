...
// 使用 gemini-2.5-flash-image：Stage 1 先畫身材基底圖 → Stage 2 再套 UNIQLO 風格穿搭
...
// ===== Stage 2：在基底圖上套 UNIQLO 風格穿搭 =====
function buildOutfitStagePrompt(gender, age, style, temp, heightCm, weightKg) {
  const styleTextMap = {
...
- Optionally add one small accessory like a crossbody bag or tote bag
- Colors: simple, clean, UNIQLO-style color palette (whites, beiges, navy, khaki, etc.)
...
- Only change clothing and small accessories, not the body.
  `.trim();
}
...
export default async function handler(req, res) {
  ...
  try {
    const { gender, age, style, temp, height, weight } = req.body || {};
    ...
    // bodyImage.base64 是我們的「藏在後端的身材基底圖」，不回前端

    // --- Stage 2：在基底圖上套 UNIQLO 穿搭 ---
    ...
    return res.status(200).json({
      ...
      // 如果你想 debug，可以順便回 prompt，但正式版建議拿掉：
