...
export default function Page() {
  ...
  function applyPresetPayload(payload: PresetPayload) {
    ...
    const ageVal = normalizeNum(payload.age);
    const heightVal = normalizeNum(payload.height);
    const weightVal = normalizeNum(payload.weight);
    const tempVal = normalizeNum(payload.temp);
  ...
  async function handleGenerate() {
    try {
      ...
      setGeneratedSummary(specObj?.summary || promptContext);
      setStage("generated");
      ...
      try {
        ...
        const slug = created?.outfit?.share_slug;
        if (slug) setGeneratedShareUrl(`${window.location.origin}/share/${slug}`);
      } catch {}
    ...
    <main className={styles.page}>
      ...
      <section className={styles.contentWrap}>
        ...
        <section className={styles.historySection}>
          <div className={styles.sectionHead}>
            <div><div className={styles.kicker}>Activity</div><h2 className={styles.sectionTitle}>最近活動</h2></div>
          </div>

          <div className={styles.historyBlock}>
            <div className={styles.historyTitle}>最近生成</div>
            {recent.length ? <div className={styles.smallRow}>{recent.map((item) => <OutfitCard key={item.id} item={item} compact onOpen={() => item.image_url && setZoomSrc(item.image_url)} />)}</div> : <div className={styles.emptyText}>目前沒有最近生成資料。</div>}
          </div>

          <div className={styles.historyBlock}>
            <div className={styles.historyTitle}>我的最愛</div>
            {favorites.length ? <div className={styles.smallRow}>{favorites.map((item) => <OutfitCard key={item.id} item={item} compact onOpen={() => item.image_url && setZoomSrc(item.image_url)} />)}</div> : <div className={styles.emptyText}>目前沒有收藏資料。</div>}
          </div>
        </section>
      </section>
