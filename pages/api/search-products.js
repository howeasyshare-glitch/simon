...
// V3.8 - Based on V3.7 + Precision Ranking
// 保留 V3.7：台灣搜尋、fallback、debug、速度優化
// 新增 V3.8：精準排序、cardigan / ribbed knit / low-top leather / custom 錯類降權
...
function categoryZh(item) {
  ...
  if (slot === "outer") {
    if (cat.includes("wool blend cardigan")) return "開襟衫 針織外套 毛衣外套 羊毛混紡 cardigan";
    if (cat.includes("cardigan")) return "開襟衫 針織外套 毛衣外套 cardigan";
  ...
  if (slot === "shoes") {
    if (cat.includes("low-top leather")) return "小白鞋 真皮 低筒休閒鞋 白色皮革休閒鞋";
...
function styleBoostWords(styleTag, slot) {
  ...
  if (s.includes("date")) {
    if (slot === "top") return "約會 簡約 質感";
    if (slot === "bottom") return "約會 修身 俐落";
    if (slot === "outer") return "約會 針織外套 質感";
    if (slot === "shoes") return "約會 小白鞋 休閒鞋";
    if (slot === "bag") return "約會 小包 斜背包";
  }
...
function getCategorySynonyms(item) {
  ...
  if (slot === "top") {
    ...
    if (cat.includes("ribbed knit")) return ["ribbed", "羅紋", "knit", "針織", "針織上衣"];
  ...
  if (slot === "outer") {
    if (cat.includes("cardigan")) return ["cardigan", "開襟衫", "針織外套", "毛衣外套", "外套"];
  ...
  if (slot === "shoes") {
    if (cat.includes("low-top leather")) return ["low-top", "leather", "小白鞋", "真皮", "皮革", "休閒鞋", "運動鞋"];
...
function passesMaterialFilter(p, item, slot) {
  ...
  if (slot === "top" || slot === "bottom" || slot === "outer") {
    ...
    if (m.includes("wool blend")) return containsAny(hay, ["wool", "羊毛", "針織", "毛衣", "knit"]);
...
function precisionBoost(p, item, slot) {
  ...
  if (slot === "top") {
    if (cat.includes("ribbed") && containsAny(t, ["羅紋", "ribbed"])) s += 6;
    if (cat.includes("knit") && containsAny(t, ["針織", "knit"])) s += 4;
    if (cat.includes("polo") && containsAny(t, ["polo", "polo衫"])) s += 5;

    if (cat.includes("ribbed") && containsAny(t, ["條紋"])) s -= 2;
    if (containsAny(t, ["工作服", "制服", "排汗", "機能衣"])) s -= 3;
  }

  if (slot === "outer") {
    if (cat.includes("cardigan") && containsAny(t, ["開襟衫", "針織外套", "毛衣外套", "cardigan"])) s += 8;
    if (cat.includes("wool") && containsAny(t, ["羊毛", "wool", "毛衣", "針織"])) s += 4;
    if (cat.includes("blazer") && containsAny(t, ["西裝外套", "西外", "blazer"])) s += 6;
    if (cat.includes("corduroy") && containsAny(t, ["燈芯絨", "corduroy"])) s += 5;

    if (cat.includes("cardigan") && containsAny(t, ["丹寧", "皮革拼接", "牛仔外套"])) s -= 10;
    if (cat.includes("cardigan") && containsAny(t, ["jacket", "夾克"]) && !containsAny(t, ["針織", "開襟", "cardigan"])) s -= 5;
  }

  if (slot === "bottom") {
    if (cat.includes("wide") && containsAny(t, ["寬褲", "寬腿", "wide leg"])) s += 6;
    if (cat.includes("jeans") && containsAny(t, ["牛仔", "denim", "jeans"])) s += 5;
    if (cat.includes("straight") && containsAny(t, ["直筒", "straight"])) s += 4;
  }

  if (slot === "shoes") {
    if (cat.includes("leather") && containsAny(t, ["真皮", "皮革", "leather"])) s += 5;
    if (cat.includes("low-top") && containsAny(t, ["低筒", "low-top", "小白鞋", "休閒鞋"])) s += 5;

    if (containsAny(t, ["厚底"])) s -= 1;
    if (containsAny(t, ["半拖", "拖鞋", "涼鞋"])) s -= 5;
  }

  if (slot === "bag") {
    if (cat.includes("crossbody") && containsAny(t, ["斜背包", "側背包", "crossbody"])) s += 5;
    if (containsAny(t, ["小包", "手機包"])) s += 2;
  }
...
export default async function handler(req, res) {
  ...
  try {
    ...
    for (const item of items) {
      ...
      const debugEntry = {
        version: "V3.8 Based on V3.7 + Precision Ranking",
        slot,
      ...
      console.log("[search-products V3.8 debug]", JSON.stringify(debugEntry));
